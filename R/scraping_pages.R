...
cbind(dt_char, dt_bid, dt_link) %>%
  mutate(
    ...
    bid = str_replace(str_replace(str_sub(end, 40, 55),',','')," .*", ""),
    
    gender = if_else(str_detect(vocation, "Male"),'Male','Female'),
    sold = if_else(str_detect(end, "Winning"),1,0),
    ...
    ref= sub(".*auctionid=", "",str_replace(ref,'&source=overview',''))
